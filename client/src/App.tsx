...
function AppContent() {
  ...
  if (!user) {
    return <AuthRouter onLogin={handleLogin} onRegister={handleRegister} authView={authView} setAuthView={setAuthView} />;
  }

  // If user is authenticated but on a public page, show public router
  if (isPublicPage) {
    return <PublicRouter />;
  }

  // If user is authenticated and not on a public page, show main layout with sidebar
  return (
