...
export default function ProductsSelectorPage() {
  ...
  return (
    <div className="flex flex-col bg-background">
      ...
      <div className="border-b border-border bg-gradient-to-b from-card via-card/95 to-card/90 px-4 py-6 flex-shrink-0">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-lg bg-pink-500/15 flex items-center justify-center flex-shrink-0 border border-pink-500/20">
              <Package className="w-5 h-5 text-pink-600 dark:text-pink-400" />
            </div>
            <div className="min-w-0">
              <h1 className="text-lg font-bold text-foreground">Gestionar Productos</h1>
              <p className="text-xs text-muted-foreground/80">Selecciona una tienda para ver, editar y gestionar sus productos</p>
            </div>
          </div>
      ...
      <div className="px-4 py-4 border-b border-border bg-card">
        <div className="max-w-7xl mx-auto">
          <div className="bg-gradient-to-r from-pink-500/10 to-pink-500/5 border border-pink-500/20 rounded-lg p-3 flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-pink-600 dark:text-pink-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-foreground">Selecciona una tienda</p>
              <p className="text-xs text-foreground/70 mt-0.5">Elige la tienda para ver, editar, agregar y gestionar sus productos</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="mb-4">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9"
                  autoComplete="off"
                  data-testid="input-search-stores-products"
                />
              </div>
            </div>
            ...
            {isLoading ? (
              ...
              <Card className="border-dashed border-2 bg-muted/20">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">Crea tu primera tienda para empezar a gestionar productos</p>
              ...
              <div className="grid gap-3">
                {filteredStores.length === 0 ? (
                  ...
                  filteredStores.map((store) => (
                    <Link href={`/stores/${store.id}/products`} asChild key={store.id}>
                    <Card
                      className="hover:border-primary/50 transition-colors cursor-pointer"
                      data-testid={`card-store-products-${store.id}`}
                    >
