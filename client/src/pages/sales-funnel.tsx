...
import { Search, TrendingUp, Users, Percent, Zap, Eye, MessageCircle, Megaphone, HelpCircle, MessageSquare, CheckCircle, AlertTriangle, ChevronDown } from "lucide-react";
            ...
            {accounts.length > 0 && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="ghost" 
                    className="h-8 px-2.5 gap-2 text-xs border border-border/60 bg-card hover:bg-card/80 hover-elevate rounded-md flex-shrink-0"
                    data-testid="select-account-funnel"
                  >
                    <span className="font-semibold text-foreground truncate max-w-[140px]">
                      {accounts.find(a => a.id === activeAccountId)?.deviceName || "Selecciona cuenta"}
                    </span>
                      ...
                      <button
                        ...
                        className={`w-full text-left px-2 py-1.5 rounded-md transition-colors flex items-start gap-2 ${
                          activeAccountId === account.id 
                            ? "bg-cyan-500/15 hover:bg-cyan-500/20 border border-cyan-500/30" 
