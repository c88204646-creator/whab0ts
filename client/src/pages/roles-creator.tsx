        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {currentRoles.map((role) => {
              ...
              return (
                <Card key={role.id} className="hover-elevate flex flex-col border border-border/40 bg-card overflow-hidden transition-all rounded-lg h-full">
                  {/* Header compacto */}
                  <div className="px-3 py-2.5 border-b border-border/30 flex items-center justify-between gap-2 bg-muted/10">
                    <div className="flex items-center gap-2.5 flex-1 min-w-0">
                      <div className={`w-7 h-7 rounded flex items-center justify-center flex-shrink-0 ${role.color} bg-opacity-15 border border-border/50`}>
                        <RoleIcon className="w-3.5 h-3.5 text-foreground/80" />
                      </div>
                      {editingRoleId === role.id ? (
                        <div className="flex gap-1 items-center flex-1 min-w-0">
                          <Input
                            ...
                            className="h-6 text-xs flex-1 py-1 px-2 bg-muted/40 border-muted/50"
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      <Button
                        ...
                        disabled={!canDelete}
                      >
                        <Trash2 className={`w-3 h-3 transition-colors ${canDelete ? "text-muted-foreground" : "text-muted-foreground/30"}`} />
                      </Button>
                    </div>
                  </div>

                  {/* Body - Contenedor con scroll */}
                  <CardContent className="flex-1 p-2.5 flex flex-col gap-2 overflow-hidden">
                    {/* Fila indicadores */}
                    <div className="flex items-center justify-between gap-2 px-1.5 py-1">
                      ...
                      <Badge 
                        variant="outline" 
                        className="text-xs px-2 py-0.5 h-5 font-medium bg-muted/20 border-border/40"
                      >
                    ...
                    {/* Módulos scrolleable */}
                    {enabledModules > 0 ? (
                      <div className="flex-1 flex flex-col gap-1 min-h-0 px-1.5">
                        <div className="flex items-center gap-1.5">
                          <Shield className="w-3 h-3 text-muted-foreground flex-shrink-0" />
                          <span className="text-xs text-muted-foreground font-medium">Accesos ({enabledModules})</span>
                        </div>
                            ...
                            {DYNAMIC_MODULES.map(module => {
                              ...
                              return perms > 0 ? (
                                <Badge 
                                  key={module} 
                                  variant="outline" 
                                  className="text-xs px-2 py-1 h-5 font-medium bg-muted/30 text-muted-foreground border-border/40 whitespace-nowrap"
                                >
                                  {module.split(' ')[0]}
                                </Badge>
                              ) : null;
                      ...
                      <div className="flex-1 flex items-center justify-center px-1.5">
                        <p className="text-xs text-muted-foreground/60">Sin accesos configurados</p>
                      </div>
                    )}

                    {/* System badge */}
                    {isDefault && (
                      <div className="mx-1.5 pt-1.5 border-t border-border/30">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="mt-auto w-full text-xs h-7 font-medium"
